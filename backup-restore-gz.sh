...
if which pigz > /dev/null; then
	...
	# -p flag for pigz sets the number of threads
...
me=$(basename "$0")
headline="
---------------------=== The backup-restore-gz script by Giuseppe Benigno <giuseppe.benigno@gmail.com> ===---------------------
"
print_usage() {
	echo "$headline"
	cat <<EOF
	...
	if [ "$type" = "dir" ]; then
		if [[ "${SYSTEM_DIRECTORIES[*]} ${WEB_DIRECTORIES[*]} ${MAIL_DIRECTORIES[*]} all" =~ "$2" ]]; then
			...
			for f_path in $farh; do
				...
				if [ -d "$f_path" ]; then
					cat "$f_path"/part-* | $TAR -z -x -f - -C "$path"
				else
					$TAR -z -x -f "$f_path" -C "$path"
				fi
			...
			for i_path in $arh; do
				...
				if [ -d "$i_path" ]; then
					cat "$i_path"/part-* | $TAR -z -x -f - -C "$path"
				else
					$TAR -z -x -f "$i_path" -C "$path"
				fi
			done
			del_res "$path" "$2" "$3" "$TMP_DIR"
		else
		...
		for d in $dblist; do
			if [ "$d" == "$2" ] || [ "$2" == "all" ]; then
				...
				for i in $arh; do
					...
					if [ -d "$i" ]; then
						cat "$i"/part-* | $TAR --gzip -xOf - | mysql --user="$DB_USER" --password="$DB_PASSWORD" --database="$rdb"
					else
						$TAR --gzip -xOf "$i" | mysql --user="$DB_USER" --password="$DB_PASSWORD" --database="$rdb"
					fi
				done
